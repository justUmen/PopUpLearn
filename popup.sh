...
function ⬚⬚⬚⬚_📃_session(){ 🔧 $FUNCNAME $@
	...
	ARG=0
	LOOP_UNTIL=`expr $SESSION_NUMBER - 1`
	while [ $ARG -ne $LOOP_UNTIL ]; do
	  ...
	  if [[ "$ERROR_TEST" != "" ]]; then
			...
			cat "$HOME/.PopUpLearn/tmp/list_lines.tmp" "$HOME/.PopUpLearn/tmp/list_mistakes.tmp" 2> /dev/null | sort | uniq -c | sed "s#^ \+1 \+\(.*\)#$GREY[\1]$END#" | sed "s#^ \+2 \+\(.*\)#$YELLOW[\1]$END#" | sed "s#^ \+3 \+\(.*\)#$ORANGE[\1]$END#" | sed "s#^ \+[0-9]\+ \+\(.*\)#$RED[\1]$END#" > "$HOME/.PopUpLearn/tmp/display_mistakes.tmp"

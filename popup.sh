...
function ⬚_before_start(){
	...
	exec 6<>/dev/tcp/127.0.0.1/9995 &> /dev/null && { echo "ERROR port 9995 is already used..."; exec 6>&- && exec 6<&- && close_PopUpLearn; }
	exec 6<>/dev/tcp/127.0.0.1/8899 &> /dev/null && { echo "ERROR port 8899 is already used..."; exec 6>&- && exec 6<&- && close_PopUpLearn; }

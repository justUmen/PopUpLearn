...
function onRequest(request, response) {
	...
	switch(code){
		...
		case "close_all_PopUpLearn": log_and_run("POPUPLEARN","if [ -f ~/.PopUpLearn/MYDB/my.config ]; then source ~/.PopUpLearn/MYDB/my.config; pkill -f \"$WEB_BROWSER http://127.0.0.1:9995\"; else pkill -f \"surf -F http://127.0.0.1:9995\"; fi");response.end();break
		...
		case "PopUpLearn_bad_answer": log_and_run("POPUPLEARN","echo bad > $HOME/.PopUpLearn/tmp/result.tmp;");response.end();break

					...
					mails.map(function(mail) {
						if(<?php if("$LANGUAGE_TAG_2"=="IM"){echo "1";}else{echo "0";} ?>){ //IF image
							if (mail.user.answer=="good") return (
									React.createElement("img", { onClick : good_answer, className: "btn glyphicon",style: {margin: "3px", src: mail.user.name}})
							);
							else return (
								React.createElement("div", { onClick : bad_answer, className: "btn glyphicon",style: {margin: "3px", src: mail.user.name}})
							);
						}
						else{
							if (mail.user.answer=="good") return (
